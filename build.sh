...
if [ -z "$DEVICE" ]; then
  ...
  echo -e "Usage: ./build.sh <device_name> [-dtbs] [-z] [-ksu]"
...
compile_dtbs() {
    ...
    local TEMP_LOG=$(mktemp)
    set -o pipefail
    make -j$(nproc --all) O="${objdir}" ARCH=${ARCH} CC=${CC} CROSS_COMPILE=$CROSS_COMPILE CROSS_COMPILE_ARM32=$CROSS_COMPILE_ARM32 KCFLAGS="-fno-use-linker-plugin" dtbs 2>&1 | tee "$TEMP_LOG"
...
completion() {
    ...
    if [[ -f "${LOCAL_BOOT}/Image.gz-dtb" ]]; then
        ...
        rm -rf "$anykernel"
        git clone --depth=1 --single-branch https://github.com/SHANDZIN/AnyKernel3.git -b "$DEVICE" "$anykernel"
...
# DTBs only (any device) — skips kernel image and packaging
if [ "$DTBS_FLAG" = true ]; then
    compile_dtbs
    DIFF=$SECONDS
